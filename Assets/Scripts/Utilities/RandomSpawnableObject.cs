    ...
    public T GetItem()
    {
        ...
        foreach (SpawnableObjectByLevel<T> spawnableObjectByLevel  in spawnableObjectByLevelList)
        {
            if(spawnableObjectByLevel.dungeonLevel == GameManager.Instance.GetCurrentDungeonLevel())
            {
                ...
                {
                    int lowerBoudary = upperBoundary + 1;

                    upperBoundary = lowerBoudary + spawnableObjectRatio.ratio - 1;
                    ...
                    chanceBoundariesList.Add(new chanceBoundaries()
                    {
                        ...
                        lowBoundaryValue =
                        lowerBoudary,
                        highBoudaryValue = upperBoundary
                    });
                }
            }
        }
        ...
        {
            if(lookUpValue >= spawnChance.lowBoundaryValue && lookUpValue <= spawnChance.highBoudaryValue)
            {
