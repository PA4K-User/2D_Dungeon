...
public class RoomNodeGraphEditor : EditorWindow
{
    ...
    private void DeleteSelectedRoomNodes()
    {
        ...
        {
            if(roomNode.isSeleted && !roomNode.roomNodeType.isEntrance)
            {
    ...
    private void DeleteSelectedRoomNodeLinks()
    {
        foreach (RoomNodeSO roomNode in currentRoomNodeGraph.roomNodeList)
        {
            if(roomNode.isSeleted && roomNode.childRoomNodeIDList.Count > 0)
            {
                for (int i = roomNode.childRoomNodeIDList.Count -1; i >= 0; i--)
                {
                    RoomNodeSO childRoomNode = currentRoomNodeGraph.GetRoomNode(roomNode.childRoomNodeIDList[i]);

                    if(childRoomNode != null && childRoomNode.isSeleted)
                    {
    ...
    private void ClearAllSelectedRoomNodes()
    {
        foreach (RoomNodeSO roomNode  in currentRoomNodeGraph.roomNodeList)
        {
            if (roomNode.isSeleted)
            {
                roomNode.isSeleted = false;
    ...
    private void DrawRoomConnentions()
    {
        foreach (RoomNodeSO roomNode in currentRoomNodeGraph.roomNodeList)
        {
